                ...
                <div class="flex flex-center">
                    <div class="w-full p-6 bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none">
                        <div class="flex items-center justify-between mt-4 mb-3">
                            ...
                            <a href="{{ URL::tokenRoute('collections.add') }}" class="bg-blue-500 text-white hover:bg-blue-700 hover:text-gray-300 focus:outline-none focus:ring focus:border-blue-300 ml-auto py-2 px-4 rounded inline-flex items-center">
                        ...
                        <table class="min-w-full text-gray-900 dark:text-white border border-gray-500 dark:border-gray-900 rounded-md overflow-hidden">
                            ...
                            <tr>
                                <td class="py-2 px-4 border-b border-gray-500 dark:border-gray-900">
                                    <a href="{{ URL::tokenRoute('products.index', [ $collection->id ]) }}" title="View {{ $collection->name }} collection's products">{{ $collection->name }}</a>
                                </td>
                                <td class="py-2 px-4 border-b border-gray-500 dark:border-gray-900">{{ $collection->description }}</td>
                                <td class="py-2 px-4 text-center border-b border-gray-500 dark:border-gray-900">
                                    <span class="rounded-2xl px-2 font-light text-sm border {{($collection->status? 'border-green-300 bg-green-200 text-green-600' : 'border-red-300 bg-red-200 text-red-600')}}">{{ ( $collection->status ? 'Active' : 'Inactive' ) }}</span>
                                </td>
                                <td class="py-2 px-4 border-b border-gray-500 dark:border-gray-900">
                                    <a href="{{ URL::tokenRoute('collections.edit', [$collection->id] ) }}" class="text-blue-500 hover:text-blue-700 focus:outline-none focus:ring focus:border-blue-300 px-2">Edit</a>
                                    @if($collection->status)
                                    <a href="{{ URL::tokenRoute('products.add', [$collection->id] ) }}" class="text-blue-500 hover:text-blue-700 focus:outline-none focus:ring focus:border-blue-300 px-2">Add Product</a>
                                    @endif
                                    <a href="{{ URL::tokenRoute('collections.delete', [$collection->id] ) }}" class="text-red-500 hover:text-red-700 focus:outline-none focus:ring focus:border-red-300 px-2">Delete</a>
                                </td>
                            </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="py-2 px-4 text-center border-b border-gray-500 dark:border-gray-900">
                                        Nothing found.
                                        <a class="text-blue-500 hover:text-blue-700" href="{{ URL::tokenRoute('collections.add') }}">Add new?</a>
                                    </td>
                                </tr>
                            @endforelse
                            </tbody>
